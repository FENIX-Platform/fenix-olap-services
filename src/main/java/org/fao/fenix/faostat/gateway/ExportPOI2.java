...
public class ExportPOI2 {
    ...
    public Response getHtml(
            ...
            @FormParam("notreeview") String notreeview
            ) throws IOException {
boolean mynotreevew=true;
try {if(notreeview.equals("1")){mynotreevew=false;}}
catch(Exception er){}
...
if(swFlag.equals( "1")){ 
         ...
         try{  
                    if (stop
                        ...
                        catch(Exception ex){ System.out.println("ho O "+stop+" "+i+"  "+Oldhead.length+" "+j);  }
                    }
    ...
    System.out.println("ET O "+stop+" "+i+"  "+Oldhead.length+" "+j);
        ...
        row = sheet.createRow(i++);
        if(footer!=null){ row.createCell((short) 0).setCellValue(footer);}
        else{
